...
int hide_byte(unsigned char pixdata[], int n, int *p_pos, unsigned char b, char bitParPixel)
{
    if(bitParPixel!=1 || bitParPixel!=2 || bitParPixel!=4 || bitParPixel!=8) return 0;
    int m=8/bitParPixel;
    if(*p_pos+m>=n) return 0;
    int j;
    unsigned char mask = 0x00;
         if(bitParPixel==1) mask = mask|0x80;
    else if(bitParPixel==2) mask = mask|0xC0;
    else if(bitParPixel==4) mask = mask|0xF0;
    else if(bitParPixel==8) mask = mask|0xFF;
    unsigned char c;
    for(j=0;j<m;j++)
    {
        c = (b&mask);
